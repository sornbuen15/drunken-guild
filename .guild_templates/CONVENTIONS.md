...
3. If an issue requires Boss approval and the Boss is reading the conversation, ask them there. Otherwise call `request_boss_approval_async` (`drunken-discord-mcp`) with `action`, `reason`, and `ticket_key` -- it returns a `req_id` immediately rather than an answer. Park the task with `board_block_task`, carry on with something unblocked, and collect the verdict with `check_approvals` when you finish a task or start a session, never mid-task.
