...
3. If an issue requires Boss approval, call the `request_boss_approval` MCP tool (`drunken-discord-mcp`) with `action`, `reason`, and `ticket_key` -- it blocks until the Boss reacts on Discord and returns the answer.
