...
- Otherwise call `request_boss_approval_async` (from `drunken-discord-mcp`) with `action`, `reason`, and `ticket_key`. It returns a `req_id` immediately, not an answer. Don't perform the action yet, take up the next unblocked task instead (there is no local board, so "parking" a task is just not doing that step, not a tool call), and collect the verdict with `check_approvals` **when you finish a task or start a session -- never mid-task.** Asking must never stop the other work, and a half-applied approval leaves the repo in a state nobody can reason about.
