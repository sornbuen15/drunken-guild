...
| `skills-no-flow/` — skills `python-quality-gates`, `project-hygiene`, `confluence-sync`, `zero-defect-mindset`, `ai-output`; `scripts/confluence_bridge.py` and `tests/test_confluence_bridge.py` | none serves a step of the 2.0.0 flow (/prd → /clarify → /ddd → /breakdown → /build → /audit). `python-quality-gates` hard-coded this repo's layout into a skill installed everywhere (DG-347); `project-hygiene` already deferred every rule to `git-workflow`; `confluence-sync` could not run outside this repo and hard-coded one project's ADRs (DG-346); the two mindset skills restated what the build step does. Groups 2 and 3 of the re-scope, approved by the Boss 2026-09-11 (DG-352). The fifteen general skills that moved in the same change are *not* retired — they live unchanged in `plugins/drunken-extras/skills/` | a project's own build/test/lint commands in its `AGENTS.md`; `git-workflow`; the `/build` step | `fcc52de`, at their original `skills/` and `scripts/` paths || `scripts/` — `setup_git_hooks.py` | it installed a local `pre-push` hook refusing a push to `main` or `develop`. The GitHub rulesets on both branches already require a pull request and passing checks, block force-push and deletion, and list **no bypass actors** — the server refuses the push whatever the local hook does. It duplicated a control that cannot be bypassed with one that can, by not being installed. `pre-commit install` is a different thing and is still required (DG-354) | the rulesets on `main` and `develop`; `pre-commit install` for the local gates | `3451136`, at `scripts/setup_git_hooks.py` |
| `skills-superseded-by-the-flow/` — `spec-to-backlog`, `backlog-refinement`, `task-estimation`, `issue-intake`, `audit-to-backlog`, `local-progress-reporter`, `project-audit-reviewer`, `test-report-generator`, `core-engineering`, `anti-regression` | the 2.0.0 flow was one command per step — six at the time, seven since DG-386 added `/replan` — and these ten were built around four that no longer exist — `/init-project`, `/refine`, `/estimate`, `/report` — plus four mindset skills that restated what a build step does. Nothing in them is lost: the backlog generation, the urgency-from-MoSCoW mapping, the sequence-what-shares-files rule, the two-axes rule, the estimate-lives-nowhere rule, the blast-radius check, the debugging mantra and the seen-failing-first rule are all carried into the six. What changed is that they are now **one file per step of a flow** instead of ten skills that each had to be discovered (DG-353) | `skills/flow/` — `prd`, `clarify`, `ddd`, `breakdown`, `build`, `audit` | `3451136`, at their original `skills/` paths |

| `discord-machinery/` — `src/service/` (listener, router, runner, utils, approval manager, daemon client), `src/discord_mcp/`, `src/core/away.py`, `scripts/jira_bridge.py`, `scripts/qa_automation.py`, `scripts/setup_daemon_service.py`, `scripts/check_pending_approval.py`, and sixteen test modules; and, edited down in place, `approval_hook.py` → `src/core/hook.py` | about 3,000 lines existed to carry one question to a phone and one answer back. The 2.0.0 target answers it differently: Remote Control is where a Claude session is watched, Jira and pull requests are the shared screen for everything else, and Discord becomes one-way notification — a line and a link for a PR being ready, today's MVP, audit gaps, or an agent needing a decision. What the hook keeps is the half that was never about asking anyone: the deny floor, which refuses what `.claude/settings.json` denies and refuses a call it cannot read, and is silent otherwise. Group 5 of the 2.0.0 re-scope, approved by the Boss 2026-09-12 (DG-355). `promote_permissions.py` is deliberately kept: it is usable by hand | `src/core/notify.py` for notifications; `src/core/hook.py` (`drunken-hook`) for the floor; asking the Boss in the conversation for approval; the `drunken-jira-mcp` tools for what the bridge script did; `/audit` for the QA gate | `7d5b9d1` for `approval_hook.py` and its tests, at their original paths; `37e711b` for everything else |
| `cli-shrink/` — `src/core/status.py` and `tests/test_core_status.py`; and, edited out in place, `config_gen`'s whole CLI (`build_parser`, `main`, `_emit_install`) | ten console scripts shipped once, and group 7 of the re-scope brings it to four an operator types plus two pieces of plumbing nobody does (DG-356). `drunken-status` re-derived what `/audit` reports, from the same sources, with a second opinion about what "where things stand" means. `drunken-config` had three jobs and lost one outright: the MCP configuration it generated is now a constant, because the project moved into every tool call (DG-341), and a constant needs no generator. The other two were moved rather than dropped — merging into a host's own config file, and the pinned install line that `uv tool install` needs because it ignores `uv.lock` | `/audit`, which gained the state-of-the-work step and the rule that "could not ask" is not an empty answer; `scripts/install/install_mcp.sh`, which prints the constant; `onboard_project.py --merge-mcp-config` for the host merge; `drunken-doctor --requirements` for the pins, so the tool that reports the drift is the one that prints the remedy. The `config_gen` module itself stays as the library all of that calls | `472f299`, at `src/core/status.py` and `tests/test_core_status.py` |
