...
- **ISAC** — create a new **Software / Kanban** project, then move the 3 open issues (ISAC-3, ISAC-4,
  ISAC-132). DT-237's stated cost of 170 issues is **wrong**: TWA has 0 open, ISAC has 3. Fix that
  ticket. Requirements memo is **ISAC-132**.
...
**Antigravity** — TWA's `.agents/AGENTS.md` still tells it to run the vendored scripts. Those
...
3. **ISAC's registry `git_root` points at a directory that is not a repository.** The warning is
