...
| Registry | `~/.drunken/projects.json` — `drunken-guild` added alongside `drunken-team`, `isac`, `twa`. Backup at `projects.json.bak-*` |
...
printed `OK … (project TWA)` while Jira answered *"No project could be found"*. A direct
...
- **The DT-250 wrapper-layout exception.** `drunken-team`'s rule says the wrapper is not a git
  ...
  **is** the product, and applying it literally moves the deliverable out of version control. TWA
