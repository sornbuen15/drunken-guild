...
- **Time pressure context (carried over):** grant funding deadline for `isac` (~/Projects/isac) was ~14 days out as of 2026-07-18. This session ran long and crossed into 2026-07-19. Round-1 is now essentially closed out -- next session should confirm with the Boss whether to start round-2 on drunken-team or pivot to `isac`.
...
1. Round-1 is now down to just **DT-94** (needs a Boss scoping conversation, not more solo coding) and **DT-90** (docs reconciliation -- straightforward, can be picked up directly). Ask the Boss which first, or whether to start round-2 / pivot to `isac`.
