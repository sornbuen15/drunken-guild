...
### TWA and ISAC are usable now

Both are software projects with boards, and both were probed rather than assumed:

| | Jira | board | git |
|---|---|---|---|
| `twa` | TWA | 141, backlog | `tff-web-app/twa` |
| `isac` | ISAC | 140, backlog | `isac/isac/isac-backend` |
...
Two things §6 and §3 used to warn about are no longer true: TWA's `.env` is untracked and
gitignored and nothing reads it since DT-254, and `twa/scripts/` holds two unrelated
...
- **DT-260** — do it or close it. `drunken-doctor` verifies the credential (`/myself`) and
  never that the project key exists, so it printed `OK … (project TWA)` while Jira answered
  `No project could be found with key 'TWA'`. The immediate cause is fixed; the blind spot
  is not.
...
**Dropped on 2026-08-19, by the Boss:** ISAC's project migration, DT-237, DT-248 and
...
3. **A Jira search cannot tell you a project exists.** `TWA` was renamed and our registry kept
   the dead key; searches returned HTTP 200 with an empty list and `drunken-doctor` printed
   `OK … (project TWA)`, because it verifies the credential and never the project. Both are
   fixed, the blind spot is DT-260. ISAC's `git_root` warning is also gone — it was pointing two
...
## 6. ISAC — the survey that has to survive

The application source **was never in `sornbuen15/isac`**. On the remote, `isac-backend` is a
...
| where | what | history |
|---|---|---|
| `sornbuen15/isac` | wrapper + AI layer + dangling gitlink | 77 commits, carries the inert token |
| `~/Projects/new-isac-parked/isac-backend` | **the application** | 67 commits, HEAD `69c85d9`, working tree emptied |
| `~/Projects/isac/isac/isac-backend` | the application, flattened | 1 commit, no remote |
| `~/.gemini/history/isac` | Antigravity's. **Do not touch** | — |
...
| §14 ISAC's five-step plan | Superseded by §6 above — the old steps would fail against what is on disk |
