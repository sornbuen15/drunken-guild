...
foreach ($AgentFile in $AgentFiles) {
    ...
    #
    # That run did surface a separate drift from install_agents.sh, which is
    # DG-280's disease rather than DG-378's and is not fixed here: this script
    # writes a BOM and CRLF, leaves the trailing blank line `end-of-file-fixer`
    # strips, separates name from description with `-` where the shell version
    # uses an em-dash, truncates with Substring rather than the shared
    # _truncate.py so a 200-character cut keeps the space the shell one strips,
    # and stamps its own filename into the "Generated by" header. So a Windows
    # run and a macOS run of the same tree still rewrite each other's
    # agents/INDEX.md, which is committed.
    #
